...
function loadGeoJSONLayer(map, geojsonFile = 'map.geojson') {
    ...
    map.on('load', () => {
        ...
        map.addLayer({
            ...
            paint: {
                'circle-radius': 6,
                'circle-color': '#FFA500', // Naranja para diferenciar
                'circle-stroke-width': 2,
                'circle-stroke-color': '#FFFFFF'
            },
        ...
        map.addLayer({
            ...
            paint: {
                'line-color': '#0000FF', // Azul
                'line-width': 4
            },
            filter: ['==', '$type', 'LineString']
        });

        // Añadir capa para MultiLineString
        map.addLayer({
            ...
            paint: {
                ...
                'line-width': 4,
                'line-dasharray': [1, 1] // Línea discontinua para diferenciar
            },
            filter: ['==', '$type', 'MultiLineString']
        });

        // Añadir capa para Polygon
        map.addLayer({
            id: 'polygon-layer',
            type: 'fill',
            source: 'geojson-data',
            paint: {
                'fill-color': '#00FF00', // Verde
                'fill-opacity': 0.5,
                'fill-outline-color': '#006400' // Verde oscuro para el borde
            },
            filter: ['==', '$type', 'Polygon']
        });

        // Añadir capa para MultiPolygon
        map.addLayer({
            ...
            paint: {
                ...
                'fill-opacity': 0.5,
                'fill-outline-color': '#B8860B' // Marrón para el borde
            },
        ...
        .then(data => {
            if (data.features && data.features.length > 0) {
                ...
                if (data.features.length === 1 && data.features[0].geometry.type === 'Point') {
                    ...
                    // Si se encontraron límites válidos, ajustar el mapa
                    if (bounds) {
                        ...
                        console.log('No se pudieron calcular los límites para los datos');
                    }
                }
            }
        })
